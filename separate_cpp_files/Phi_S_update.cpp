...
cx_cube::fixed<N_static, K_static, F_static> outtensor_real_NKF;
...
outtensor_real_NKF.slice(f_index).row(n_index)=outtensor_real_NKF.slice(f_index).row(n_index)+((*Xhat_low_fnm_p)(f_index, n_index, m_index))*((*expj_Phi_S_nkf_p).slice(f_index).row(n_index))%((*T_fk_p).row(f_index)%(*V_nk_p).row(n_index))%((Xhat_outtensor_real_FLM.slice(m_index).row(f_index))*(*Y_lk_p));
...
/*outtensor_target_cx_NKF.slice(f_index).row(n_index)=-((conj((*expj_Phi_S_nkf_p).slice(f_index).row(n_index)))%outtensor_target_cx_NKF.slice(f_index).row(n_index)+2*outtensor_real_NKF.slice(f_index).row(n_index));*/

/*outtensor_target_cx_FKN.slice(n_index).row(f_index)=outtensor_target_cx_NKF.slice(f_index).row(n_index);*/

/*Cosine method*/
outtensor_target_cx_NKF.slice(f_index).row(n_index)=outtensor_target_cx_NKF.slice(f_index).row(n_index)+outtensor_real_NKF.slice(f_index).row(n_index);
...
(*(argStruct_p->expj_Phi_S_nkf_p))=outtensor_target_cx_NKF/abs(outtensor_target_cx_NKF);
(*(argStruct_p->expj_Phi_S_fkn_p))=outtensor_target_cx_FKN/abs(outtensor_target_cx_FKN);
...
(*(argStruct_p->expj_Phi_S_nkf_p))=(*(argStruct_p->expj_Phi_S_nkf_p))/abs((*(argStruct_p->expj_Phi_S_nkf_p)));

//FKN update below
(*(argStruct_p->expj_Phi_S_fkn_p))=(*(argStruct_p->expj_Phi_S_fkn_p))/abs((*(argStruct_p->expj_Phi_S_fkn_p)));*/
