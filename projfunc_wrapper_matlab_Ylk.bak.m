...
for k=1:K_static
    ...
    k1_current=norm(y_col_k, 1);
    
    current_sparseness=(sqrt(L_static)-(k1_current/k2))/(sqrt(L_static)-1);
    
    %if the current sparseness is less than 0.75, then compute a sparseness
    %that is 10% sparser than its current sparseness. 
    
    if (current_sparseness<=0.6)
        
            target_sparseness=current_sparseness+0.15;
            compute_flag=1;
            
    elseif (current_sparseness <0.75)
            
            target_sparseness=0.75;
            compute_flag=1;
    ...
    k1=k2*(sqrt(L_static)-target_sparseness*(sqrt(L_static)-1));
    
    [v, usediters]=projfunc_local(y_col_k, k1, k2, 1);
