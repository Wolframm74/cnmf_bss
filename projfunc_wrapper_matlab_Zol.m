function [Z_ol]=projfunc_wrapper_matlab_Zol(Z_ol)

display('INSIDE PROJFUNC_WRAPPER_MATLAB_ZOL!!!!!!!!!!!!!!!1');
...
for l=1:L_static
    ...
    display([ 'Line 16: The current value of l= ', num2str(l) ]); 
    
    z_col_l=squeeze(Z_ol(:,l));
    k2=norm(z_col_l,2);
    
    k1_current=norm(z_col_l, 1);
    
    current_sparseness=(sqrt(O_static)-(k1_current/k2))/(sqrt(O_static)-1);
    
    %if the current sparseness is less than 0.6, then compute a sparseness
    %that is 10% sparser than its current sparseness. 

    display([ 'Line 28: The current value of l= ', num2str(l) ]);     
    ...
    display([ 'Line 51: The current value of l= ', num2str(l) ]); 
    ...
    display([ 'Line 58: The current value of l= ', num2str(l) ]);         
    ...
    k1=k2*(sqrt(O_static)-target_sparseness*(sqrt(O_static)-1));

    display([ 'Line 65: The current value of l= ', num2str(l) ]);     
    
    [v, usediters]=projfunc_local(z_col_l, k1, k2, 1);

    display([ 'Line 69: The current value of l= ', num2str(l) ]);     
    
    Z_ol(:,l)=v;
    
    end

    display([ 'Line 75: The current value of l= ', num2str(l) ]); 
