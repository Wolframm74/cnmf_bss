...
cx_cube::fixed<F_static, L_static, N_static> expj_Phi_U_flnm[M_static];
...
switch (which_update){
    case 1:
      ...
      Xhat_dummytensor_cx_FOM[iter_ctr].zeros();

      }

      }    

      start_rtn=Xhat_primary_auxfun_start;
      ...
      strcpy(argStruct_p->update_string,"Xhat_primary_auxfun_start");

      /*Can disable this if you don't need it*/
      which_update_helper=which_update;

      Xhat_serial_function(argStruct_p->Z_ol_p, argStruct_p->W_fom_cx_p, argStruct_p->W_fom_p);

      break;
    case 2:
      start_rtn=T_primary_auxfun_start;
      dim1_len=(double)F_static;  
      dim2_len=(double)K_static;

      dim_flag=true; 

      strcpy(argStruct_p->update_string,"T_primary_auxfun_start");      

      break;
    case 3:
      start_rtn=V_primary_auxfun_start;
      dim1_len=(double)N_static;  
      dim2_len=(double)K_static;

      dim_flag=true; 

      strcpy(argStruct_p->update_string,"V_primary_auxfun_start");      

      V_0th_sum_preprocess(argStruct_p->V_nk_p);

      break;                  
    case 4:
      start_rtn=W_primary_auxfun_start;
      dim1_len=(double)F_static;  
      dim2_len=(double)O_static;

      dim_flag=true; 

      strcpy(argStruct_p->update_string,"W_primary_auxfun_start");      
      ...
      break;
    case 5:
      start_rtn=Phi_S_primary_auxfun_start;
      dim1_len=(double)F_static;  
      /*dim1_len=(double)N_static;  */
      dim2_len=(double)N_static;
      ...
      strcpy(argStruct_p->update_string,"Phi_S_primary_auxfun_start");      

      Phi_S_update_auxfun1_zero_internal_quantities();

      break;
    case 6:

      start_rtn=Z_primary_auxfun_start;
      dim1_len=(double)O_static;  
      dim2_len=(double)L_static;

      dim_flag=true; 

      strcpy(argStruct_p->update_string,"Z_primary_auxfun_start");      

      break;
    case 7:

      start_rtn=Y_primary_auxfun_start;
      dim1_len=(double)M_static;  
      dim2_len=(double)F_static;

      dim_flag=true; 

      strcpy(argStruct_p->update_string,"Y_primary_auxfun_start");      
