...
rowvec::fixed<N_static> dummy_rowvec_real_f_1xN;
rowvec::fixed<N_static> second_dummy_rowvec_real_f_1xN;

cx_mat::fixed<N_static, K_static> dummy_mat_cx_NK;

rowvec::fixed<K_static> dummy_rowvec_real_1xK;
cx_rowvec::fixed<N_static> P_Xhm_wrt_Wfom_1xN;
rowvec::fixed<N_static> P_Gm_wrt_Wfom_1xN;
rowvec::fixed<N_static> P_Hm_wrt_Wfom_1xN;
rowvec::fixed<N_static> P_Hm_wrt_Wfom_num_1xN;
rowvec::fixed<N_static> P_arg_Xhm_wrt_Wfom_1xN;

mat::fixed<F_static, O_static, M_static> Pwrt_C_Wfom_fom;

/*populate_G_H_f_1xN_channel_m local*/

rowvec::fixed<N_static> Gm_outrow_f_1xN;
rowvec::fixed<N_static> Hm_outrow_f_1xN;

rowvec::fixed<N_static> sqrt_den_outrow_m_f_1xN;

void W_update2_Wfom(cube* W_fom_p){
...
for (m_index=0; m_index<M_static; m_index++){
	...
	for (f_index=0; f_index<F_static; f_index++){
		...
		for (o_index=0; o_index<O_static; o_index++){
			...
			dummy_mat_cx_NK=((*expj_Phi_W_fom_p)(f_index, o_index, m_index))*((*expj_Phi_S_nkf).slice(f_index));
