...
mat::fixed<L_static, K_static> Partial_wrt_Ylk;

static mat::fixed<F_static, N_static> dummy_mat_local1_fn[NUM_WORKER_THREADS];
static mat::fixed<F_static, N_static> dummy_mat_local2_fn[NUM_WORKER_THREADS];

static void compute_Partial_Covariance_Cost_wrt_Ylk(mat* T_fk_p, mat* V_nk_p, mat* Z_ol_p, int l_index, int k_index, int thread_iter){

int o_index;

double dummy_out=0;

dummy_mat_local2_fn[thread_iter]=kron( (*T_fk_p).col(k_index) , trans((*V_nk_p).col(k_index)) );
...
for (o_index=0; o_index<O_static; o_index++){
	...
	dummy_mat_local1_fn[thread_iter]=((*Z_ol_p)(o_index, l_index))*(dummy_mat_local1_fn[thread_iter]);

	dummy_mat_local1_fn[thread_iter]=dummy_mat_local1_fn[thread_iter]%dummy_mat_local2_fn[thread_iter];

	dummy_out=dummy_out+accu(dummy_mat_local1_fn[thread_iter]);
		
}

Partial_wrt_Ylk(l_index, k_index)=-dummy_out;
...
static void compute_Partial_Covariance_Cost_wrt_Ylk_start(arg_struct_t* argStruct_p, int thread_iter){
...
while (threads_while_condition_1st_sum_flag&&(!(last_thread_to_sleep_flag_array[1][thread_iter]))){
	...
	compute_Partial_Covariance_Cost_wrt_Ylk(argStruct_p->T_fk_p, argStruct_p->V_nk_p, argStruct_p->Z_ol_p, l_index, k_index, thread_iter);
...
void* compute_Partial_Covariance_Cost_wrt_Ylk_thread_start(void* arg){
...
compute_Partial_Covariance_Cost_wrt_Ylk_start(argStruct_p, thread_iter);
